...
app.get("/delete", (req, res) => {
  if (req.isAuthenticated()) {
    Blog.find({ username: req.user.username }).then(foundBlogs => {
      res.render("delete", { blogs: foundBlogs });
    });
  } else {
    res.redirect("/login");
  }
...
//     // console.log(found);
//     if (found.length != 0)
//       res.render("type", { title: "Blogs found", found: found, mine: false });
//     else res.render("soon");
//   };
//   find();
// });
...
app.post("/delete", (req, res) => {
  ...
  Blog.findOneAndDelete({ _id: blogId, username: req.user.username })
    .then(() => {
      res.redirect("/Mine");
    })
    .catch(err => {
